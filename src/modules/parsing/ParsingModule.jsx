...
/** Целые секунды ≥ 1, без верхней границы (совпадает с API и telethon_parser). */
const normalizeJoinWaitSeconds = (value, fallback = 20) => {
...
const parsePeriodDays = (value, fallback = 30) => {
  const t = String(value ?? "").trim();
  if (t === "") return fallback;
  return clamp(t, 1, 365, fallback);
};
...
const ParsingModule = () => {
  ...
  const startParsing = async () => {
    ...
    const safeDays = parsePeriodDays(activeLastDays, 30);
    const safeJoinWait = normalizeJoinWaitSeconds(joinWaitSeconds, 20);
    setActiveLastDays(String(safeDays));
    setJoinWaitSeconds(String(safeJoinWait));
  ...
  useEffect(() => {
    ...
    const t = setInterval(async () => {
      try {
        ...
        if (!s.running) { setIsParsing(false); if (s.status === "done") { const finalData = await api.getParsingResults({ sourceLink, periodDays: parsePeriodDays(activeLastDays, 30), premiumFilter }); const m = mapRows(finalData); setResults(m); setLogs((p) => [`Готово: найдено ${m.length} уникальных пользователей.`, ...p]); } else if (s.status === "error") setLogs((p) => [`Ошибка Telethon: ${s.error || "unknown"}`, ...p]); }
  ...
  const filteredResults = useMemo(() => {
    const days = parsePeriodDays(activeLastDays, 30);
    let n = results.filter((r) => r.lastSeenDays <= days);
    if (premiumFilter === "premium") n = n.filter((r) => r.isPremium);
    if (premiumFilter === "non_premium") n = n.filter((r) => !r.isPremium);
    const s = new Set();
    return n.filter((r) => (s.has(r.id) ? false : (s.add(r.id), true)));
  }, [results, activeLastDays, premiumFilter]);
